...
function intentar() {
    ...
    if (INTENTO === palabra) {
        mostrarPalabra(palabra, "PINK"); // Mostrar la palabra en verde si es correcta
    ...
    intentos--;
    if (intentos === 0) {
        terminar("<h1>¡PRUEBA CON OTRA PALABRA!</h1>");
    }
}
...
function mostrarPalabra(palabra, color) {
    ...
    for (let i in palabra) {
        ...
        SPAN.style.backgroundColor = "PURPLE"; // Establecer el color
        ROW.appendChild(SPAN);
    }
    document.getElementById("grid").appendChild(ROW);
    document.getElementById("grid").appendChild(document.createElement('br'));
